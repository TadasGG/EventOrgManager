  ...
  <div>
    <header>
      <div class="primary-color rounded-xl shadow-lg p-2">
        <h1 class="text-center" style="font-family: 'Arial'">{{ t('titles.itemCategories') }}</h1>
      </div>
    </header>
    ...
    <main class="grid gap-6 my-10">
      ...
      <div class="secondary-color rounded-xl shadow-lg p-4 w-full">
        <div class="flex justify-end">
          <UButton
              class="rounded-xl"
              :to="{ path: '/items/categories/new', query: { callback: route.fullPath } }"
...
type CategoryRow = Category & { actions?: null }
...
async function deleteCategory(categoryId: number) {
  ...
  try {
    const deleteResponse = await fetch(`${apiBase}/admin/items-categories/${categoryId}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${userToken}`
      }
...
const columns: TableColumn<CategoryRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => h('span', null, row.original.name)
  },
  {
    ...
    cell: ({ row }) =>
        h('div', { class: 'flex justify-end gap-2' }, [
          h(UButton, {
            ...
            to: {
              path: `/items/categories/${row.original.id}`,
...
const filteredCategoryList = computed(() => {
  ...
  return categoryList.value.filter(categories =>
      (categories.name.toLowerCase().startsWith(lowerSearchQuery))
  );
});
</script>
